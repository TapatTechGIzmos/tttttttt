...
const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzF9QuPkmczwX22VQdS2WETYXo80e1Hk3O7IicbCYC6nJoEd69LwBQTZUx61_TJ_ra2_eQ/exec';

const getFactorLabelByKey = (key: string): string => {
  const factorList = [
...
const getQ19Data = (data: SurveyData) => {
...
const OPTIONS_Q10_SEGMENTS = ['Mass market', 'Middle market', 'Affluent market', 'We do not offer personal lines'];

const OPTIONS_Q11_CORP_SIZE = ['Micro Enterprise: <10 employees', 'Small Enterprise: 10 to 49 employees', 'Medium Enterprise: 50 to 99 employees', 'Commercial Enterprise: 100 to 299 employees', 'Large Corporate Enterprise: 300 plus employees'];
...
const OPTIONS_Q14_LEADS = ['Buy hot leads list', 'Existing client referral', 'Leverage social media', 'Cross sell to existing client base', 'Referral from a professional networks', 'Direct Outreach - Cold calling and email', 'Partnerships and Alliances (corporates, affinity groups, government departments, etc)', 'Other (please specify)'];

const OPTIONS_Q16_REASONS = ['To save time', 'To improve customer experience', 'To reduce admin workload', 'To remain competitive', 'To lower operational costs', 'I wouldn\'t use more digital tools'];

const OPTIONS_Q18_INSURERS = ['Bryte Risk Services', 'Botswana Insurance Company', 'Hollard Insurance Company', 'Liberty General Botswana', 'Old Mutual Short-Term Insurance', 'Phoenix of Botswana Assurance', 'Sesiro Insurance Company', 'Sunshine Insurance Company', 'Western Insurance Botswana'];
...
export const submitSurvey = async (
  ...
  const applyBinary = (optionsArray: string[], dataField: string[] | undefined, prefix: string) => {
    optionsArray.forEach(option => {
      binaryData[`${prefix}: ${option} (Selected)`] = dataField?.includes(option) ? 1 : 0;
    });
  };

  applyBinary(OPTIONS_Q6_DISTRICTS, data.districts, 'Q6');
  applyBinary(OPTIONS_Q8_SERVICES, data.services, 'Q8');
  applyBinary(OPTIONS_Q10_SEGMENTS, data.personalLinesSegment, 'Q10');
  applyBinary(OPTIONS_Q11_CORP_SIZE, data.corporateClientSize, 'Q11');
  applyBinary(OPTIONS_Q14_LEADS, data.leadSources, 'Q14');
  applyBinary(OPTIONS_Q18_INSURERS, data.primaryInsurers, 'Q18');
  applyBinary(OPTIONS_Q26_CONCERNS, data.biggestConcerns, 'Q26');
  applyBinary(OPTIONS_Q31_COMM_PREFS, data.communicationPreferences, 'Q31');
  applyBinary(OPTIONS_Q32_SUPPORT, data.supportNeeds, 'Q32');
  ...
  applyBinary(OPTIONS_Q16_REASONS, data.digitalToolReasons, 'Q16');
  ...
  try {
    ...
    alert(`Survey results submitted for ${surveyCountry} - ${surveySourceType}!`);
  } catch (error) {
